...
const HourlyWeather = ({ hourlyData, className }) => {
  ...
  return (
    <div
      id="forecastList"
      className={`flex flex-col items-start text-sm text-center w-full p-2 rounded-xl relative bg-opacity-20 text-white bg-black backdrop-blur-3xl font-semibold overflow-scroll ${className}`}
    >
      ...
      <div className="flex gap-2 ">
        {hourlyData.list.map((hour, index) => (
          <div
            className="h-28 w-12 flex flex-col justify-between p-2 rounded-md"
            key={index}
          >
